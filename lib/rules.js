...
rules.code = function(tokens, idx /*, options, env */) {
  ...
  return '<nu-cd><textarea>' + escapeHtml(tokens[idx].content) + '</textarea></nu-cd>';
};
...
rules.heading_open = function(tokens, idx /*, options, env */) {
  return '<nu-h' + tokens[idx].hLevel + (idx ? ' padding="1em top"' : '') + '>';
};
...
rules.hr = function(tokens, idx, options /*, env */) {
  return '<nu-line></nu-line>' + getBreak(tokens, idx);
};
...
rules.paragraph_open = function(tokens, idx /*, options, env */) {
  return tokens[idx].tight ? '' : '<nu-block>';
};
...
rules.image = function(tokens, idx, options /*, env */) {
  ...
  return '<nu-img' + src + alt + title + '></nu-img>';
};
...
rules.sub = function(tokens, idx /*, options, env */) {
  return '<nu-sub>' + escapeHtml(tokens[idx].content) + '</nu-sub>';
};
rules.sup = function(tokens, idx /*, options, env */) {
  return '<nu-sup>' + escapeHtml(tokens[idx].content) + '</nu-sup>';
};
...
rules.footnote_ref = function(tokens, idx) {
  ...
  return '<nu-sup class="footnote-ref"><nu-link to="#fn' + n + '" id="' + id + '">[' + n + ']</nu-link></nu-sup>';
};
rules.footnote_block_open = function(tokens, idx, options) {
  var hr = '<nu-line class="footnotes-sep" ></nu-line>\n';
